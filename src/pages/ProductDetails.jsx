...
export const ProductDetails = () => {
  ...
  const [selectedDuration, setSelectedDuration] = useState('1 ay');
  ...
  useEffect(() => {
    ...
    const fetchProd = apiService.getProductById(id)
      .catch(() => apiService.getProductBySlug(id))
      .catch(() => null);

    const fetchAllProds = apiService.getProducts({ size: 20 })
      .then(res => res.data?.content || [])
      .catch(() => []);

    Promise.all([fetchProd, fetchAllProds])
      .then(([prodRes, allProds]) => {
        const prod = prodRes?.data || null;
        setProduct(prod);

        if (prod) {
          const defaultLabel = prod.durations ? prod.durations[0].label : prod.duration || '1 ay';
          setSelectedDuration(defaultLabel);

          const related = allProds.filter(p => (p.category === prod.category || p.categoryId === prod.categoryId) && p.id !== prod.id).slice(0, 4);
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center text-slate-500 text-xs">
        Məhsul haqqında məlumat yüklənir...
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="min-h-screen pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center space-y-4">
        <h2 className="text-2xl font-bold">Məhsul tapılmadı</h2>
        <p className="text-slate-500 text-xs">Axtardığınız rəqəmsal məhsul sistemdə mövcud deyil.</p>
        <Link to="/products" className="inline-block px-6 py-3 rounded-2xl bg-slate-900 text-white font-bold text-xs">
          Məhsullara qayıt
        </Link>
      </div>
    );
          ...
          <div className="bg-white dark:bg-[#14171D] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-sm">
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800/80">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {["100% Rəsmi aktivasiya zəmanəti", "Instant çatdırılma", "7/24 Texniki dəstək", "Müddətsiz zəmanət"].map((feat, i) => (
          ...
          <div className="sticky top-28 bg-white dark:bg-[#14171D] border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-md">
            ...
            {product.durations ? (
              ...
              <div className="text-xs text-slate-500 font-mono">
                Müddət: <strong className="text-slate-900 dark:text-white">{product.duration}</strong>
              </div>
            )}
